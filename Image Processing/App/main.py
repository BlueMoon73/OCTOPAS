...
#         super(OriginalImage, self).__init__(**kwargs)
...
class OilSpillImage:
    ...
    def process2(self, img, hsvRange1, hsvRange2):
        ...
        # final = np.uint8(final)
        # else:
        cv.putText(final, "No Oil Spills!", (0, 0), cv.FONT_HERSHEY_SIMPLEX,  50, (255, 0, 0), 3)
    ...
    def label(self, finalimg, ellipse, totalOilArea, circlerad, largestContourArea, hull, px, cm):
        ...
        # labeling
        cv.putText(finalimg, ('Total Oil Area: ' + str((totalOilAreainCM2)) + ' cm'), (0, 15),
...
class FilePickerScreen(Screen):
    ...
    def _finish_init(self):
        self.layout = self.ids["overallLay"]
    ...
    def open(self, path, fileName):
        # self.test.add_widget(self.OilImage)
        fileName
        print(fileName[0])
        print(path)
        print(len(fileName))
        if (len(fileName) == 0):
            ...
            label = Label(text="Please select a file!")
    ...
    def process(self, path, fileName):
        # file = os.path.join(path, fileName[0])
        # print("processpic"  + self.processPic)
        file = self.OilImage.source
        pic = cv.imread(file)
        self.processPic = OilSpillImage(pic)
        print("file being processed" + file)
        # pic = self.processPic.scale(pic, scale=0.5 )
        final, median, contour, maxContourArea, numOfMaxContour, maxContour, hull, totalOilArea6 = self.processPic.process2(
            pic, (17, 9, 154), (43, 27, 181))
        outerHullList = self.processPic.outerEdges(final)
        boom6, center6, radius6 = self.processPic.boom(final, hull, scale=1.1)
        ellipseBoom, ellipse = self.processPic.ellipseboom(final, hull)
        outerHull = outerHullList[0]
        outerCircle, outerCenter, outerRadius = self.processPic.boom(final, outerHull, scale=0.9)
        cm = 3
        pixelsOnScreen = outerRadius
        scale = pixelsOnScreen / cm
        print(math.pi * ((pixelsOnScreen / scale) ** 2))
        labeled6 = self.processPic.label(final, ellipse, totalOilArea6, radius6, maxContourArea, hull, pixelsOnScreen,
                                         cm)


        cv.imwrite('processed_pic.jpg', final)
        self.OilImage.reload()
        # self.OilImage.remove_from_cache()
        print('reloeaded')
        self.OilImage.source = 'processed_pic.jpg'

    def clear(self,path):
        print("cleared!")
        # filename = 'processed_pic.jpg'
        os.remove('processed_pic.jpg')
        self.OilImage.source=''
        # self.OilImage = None
